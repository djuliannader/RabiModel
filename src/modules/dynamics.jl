...
function fotoct(psi0::Vector{Complex{Float64}},fq::Matrix{Complex{Float64}},tmax::Float64,om::Float64,Nmax,L)
 ...
 for i in 1:nint
   ...
   println(io2,T*i," ",round(real(qfi),digits=8)," ",round(real(qfi2),digits=8)," ",round(real(qfi3),digits=8)," ",round(real(qfi3),digits=8))
 ...
 println("----------- Dynamics governed by the Floquet operator for the time dependent Hamiltonian       -----")
 println("             The file contains Fotoc(t) from 0 to ",tmax," in steps of ",T ," time units               ")
 println("--------------------------------------------------------------------------------------------------- ")
 println("--------------------------------------------------------------------------------------------------- ")
