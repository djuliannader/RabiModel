...
function sigmay(n)
    vdiag=[0.0+0.0*im for i in 0:n for j in -1:0]
    Sigy=Array(Diagonal(vdiag))
    for i in 1:n
      Sigy[2*i,2*i-1]=im
      Sigy[2*i-1,2*i]=-im
    end
...
function anhilation(n)
   vdiag=[0.0 for i in 0:n for j in -1:0]
   aop=Array(Diagonal(vdiag))
   for i in 1:n
       aop[2*i-1,2*i+1]=(i)^(1/2)
       aop[2*i,2*i+2]=(i)^(1/2)
    end 
   return aop
end

function Lindbladian(Ham,jpar,jop)
   Nmax = size(Ham, 1)
   Id = Matrix{ComplexF64}(I, Nmax, Nmax)
   unitaryterm = -im * (kron(Id, Ham) - kron(transpose(Ham), Id))
   dissipative = zeros(ComplexF64, Nmax^2, Nmax^2)
    for i in 1:length(jpar)
        C = jop[i]
        CdC = C' * C
        dissipative += jpar[i] * (kron(conj(C), C)- 0.5 * (kron(Id, CdC) + kron(transpose(CdC), Id)))
    end
    L = sparse(unitaryterm + dissipative)
    return L
end
